...
def process_histogram_mine(img_name,out_dir):
    ...
    img_new = cv.cvtColor(img,cv.COLOR_RGB2YCrCb)
    hist_v = cv.calcHist([img_new],[0],None,[256],[0,256]) 
    ...
    for i in range(1) :
        if i == 0 :
            equalizaed_img = process_each_channel(0,img_new,hist_v,equalization_map_v,equalization_val_list_v,equalizaed_img,height,width,L,out_dir)
        else :
            pass
...
def process_error_and_show(img,equalizaed_img,height,width,out_dir) :
    ...
    fine_tune_img_original = cv.cvtColor(img.astype(np.uint8), cv.COLOR_YCrCb2RGB)    
    fine_tune_img_opencv = cv.cvtColor(opencv_equ.astype(np.uint8), cv.COLOR_YCrCb2RGB)    
    fine_tune_img_mine = cv.cvtColor(equalizaed_img.astype(np.uint8), cv.COLOR_YCrCb2RGB)    
