import time, os, sys, torch, visdom, argparse
import numpy as np
...
parser.add_argument('--training_data', type=str,
					default='dataset_400/training_data.csv',
					help='location of training_data')
parser.add_argument('--validation_data', type=str,
                    default='dataset_400/validation_data.csv',
...
def main():
    ...
    model = build_ssd(cfg['min_dim'], cfg['num_classes'], args.num_samples)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        loss, loss_c, loss_l = train(train_loader=train_data_loader,
                            model=model,
                            criterion=criterion,
                            optimizer=optimizer,
        ...
        if epoch % 3 == 0 and epoch != 0:
            ...
            state_dict_filename = args.dataset + '_SSD_'  + '_new.pt'
            state_dict_filename_best = args.dataset + '_SSD_' + '_best.pt'
            state = {
                    'epoch': epoch,
                    'loss': loss,
                    'best_loss': best_loss,
...
def train(train_loader, model, criterion, optimizer, epoch, global_epoch, cfg):
    ...
    for i, (images, ground_truth) in enumerate(train_loader):
        ...
        # Forward prop.
        predicted_locs, predicted_scores, prior, total_kl = model(images, "train")

        # Loss  
        boxes = [i[:, :-1].cuda() for i in ground_truth]
        labels = [i[:,-1].cuda() for i in ground_truth]
        loss_l, loss_c, kl_loss = criterion(predicted_locs, predicted_scores, prior, boxes, labels, args.batch_size)
        ...
        if i % args.print_freq == 0:
            print('| Epoch: [%d / %d][%d / %d]  /  '
                  ...
                  '\n===================================================================\n' %(epoch+1, args.num_epochs, i, len(train_loader),
                                        ...
                                        ,losses_c.val ,losses_l.val,losses.val), end="")
            ...
            if args.print_to_file:
                f.write('| Epoch: [{0} / {1}][{2} / {3}]  /  '
                  ...
                  'Coordinates Loss {loss_l.val:.4f} /  '
                  'Total Loss {loss.val:.4f} '
                  '\n==================================================================='.format(epoch+1, args.num_epochs, i, len(train_loader),
                                                                  batch_time=batch_time,
                                                                  data_time=data_time, loss_c=losses_c, loss_l=losses_l, loss=losses))
        
        del loss_c, loss_l, loss, images, ground_truth, boxes, labels
        del predicted_locs, predicted_scores, prior

        torch.cuda.empty_cache()

    # return loss to save best model
    return losses.avg, losses_c.avg, losses_l.avg
...
def validate(val_loader, model, criterion, epoch, global_epoch):
    ...
    with torch.no_grad():
        ...
        for i, (images, ground_truth) in enumerate(val_loader):
            ...
            # Forward prop.
            predicted_locs, predicted_scores, prior, total_kl = model(images, "validaiton")  # (N, 8732, 4), (N, 8732, n_classes)
            # predicted_scores = F.softmax(predicted_scores, -1)

            # Loss
            boxes = [i[:, :-1].cuda() for i in ground_truth]
            labels = [i[:,-1].cuda() for i in ground_truth]
            loss_l, loss_c, kl_loss = criterion(predicted_locs, predicted_scores, prior, boxes, labels)  # scalar
            loss = loss_l + loss_c + kl_loss
            ...
            if i % args.print_freq == 0:
                print('\r| [ %d / %d][%d / %d] / '
                    'Iter Time %.3f / Coordinate loss : %.3f / Classification loss : %.3f /Total Loss : %.3f' %(epoch+1, args.num_epochs,i, len(val_loader),
                                    batch_time.val, losses_c.val, losses_l.val, losses.val),end="")
                sys.stdout.flush()
