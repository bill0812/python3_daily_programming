# print(loss_l.item(),images.size(0))import json
import os
...
from other_utils.data.config import HOME
from other_utils.data.xview import map_labels_contiguous, map_labels_name
...
label_map = map_labels_contiguous(osp.join(XVIEW_ROOT, 'xview_labels.txt'))
rev_label_map = label_map 
label_name, label_name_origin = map_labels_name(osp.join(XVIEW_ROOT, 'xview_labels.txt'))
rev_label_map_name = {v: k for k, v in label_name.items()}  # Inverse mapping

# print(len(label_map))
# Color map for bounding boxes of detected objects from https://sashat.me/2017/01/11/list-of-20-simple-distinct-colors/
distinct_colors = ['#e6194b', '#3cb44b', '#ffe119', '#0082c8', '#f58231',
                   '#911eb4', '#46f0f0', '#f032e6', '#fffac8', '#800000',
                   '#d2f53c', '#fabebe', '#008080', '#000080', '#aa6e28', 
                   '#aaffc3', '#808000', '#3cb44b', '#ffe119', '#0082c8',
                   '#ffd8b1', '#e6beff', '#808080', '#FFFFFF', '#e6194b',
                   '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#aa6e28',
                   '#d2f53c', '#fabebe', '#008080', '#000080', '#fffac8', 
                   '#800000', '#aaffc3', '#808000', '#3cb44b', '#ffe119',
                   '#ffd8b1', '#e6beff', '#808080', '#FFFFFF', '#e6194b', 
...
def calculate_mAP(det_boxes, det_labels, det_scores, true_boxes, true_labels):
    ...
    # go through each classes without 'no-class' class
    for c in range(1, n_classes):
        ...
        cumul_recall = cumul_true_positives / object_boxes.size(0)  # (n_class_detections)
    ...
    average_precisions = {label_name[list(rev_label_map.values())[c+1]]: v for c, v in enumerate(average_precisions.tolist())}
...
# current not use, but maybe future (for uncertainty)
...
def save_checkpoint(epoch, epochs_since_improvement, model, optimizer, loss, best_loss, is_best, folder_save):
    """
    ...
    state = {'epoch': epoch,
             ...
             'model': model,
             'optimizer': optimizer}
    filename = 'checkpoint_ssd_400.pt'
    torch.save(state, folder_save + filename)


    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_best:
        torch.save(state, folder_save + 'BEST_' + filename)
