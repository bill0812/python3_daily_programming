...
def train(train_loader, model, criterion, optimizer, epoch, global_epoch, cfg):
    ...
    for i, (images, ground_truth) in enumerate(train_loader):
        ...
        # Forward prop.
        predicted_locs, predicted_scores, prior = model(images)
        ...
        del loss_c, loss_l, loss, images, ground_truth, boxes, labels, det_boxes_batch, det_labels_batch, det_scores_batch
        del predicted_locs, predicted_scores, prior

        torch.cuda.empty_cache()

    # return loss to save best model
    return losses.val, losses_c.val, losses_l.val
...
def validate(val_loader, model, criterion, epoch, global_epoch):
    ...
    with torch.no_grad():
        ...
        for i, (images, ground_truth) in enumerate(val_loader):
            ...
            # Forward prop.
            predicted_locs, predicted_scores, prior = model(images)  # (N, 8732, 4), (N, 8732, n_classes)
